...
export default {
  ...
  heightFromGroundInCm: (val = '') => {
    ...
    return (!Number.isInteger(val) && `${val}`.length <= 100)
  ...
  passengersCapacity: (val, form, extra) => {
    if (extra?.disabled) {
      return false
    }
    return (!val && val !== 0) ? t.error('required').replace('$$param', t.order('palletsCapacity')) : false
  },
  ...
  category: (val) => {
    return !val || !val?.length ? t.error('required').replace('$$param', t.transports('table.categoryType')) : false
