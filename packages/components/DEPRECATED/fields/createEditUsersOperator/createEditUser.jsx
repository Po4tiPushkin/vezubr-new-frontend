...
import { Profile as ProfileService, Contragents } from '@vezubr/services/index.operator';
...
class CreateEditUser extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data: _cloneDeep(Static.operatorUserFields),
      isMe: false,
      errors: {
        type: false,
        name: false,
  ...
  async setEditableUserData() {
    const { user, store, contractorId } = this.props;
    const { data, userRoles, digitalSignatureType } = this.state;
    const userData = (await Contragents.user({ userId: user.id, contractorId })).data;
  ...
  async validatePasswordFields() {
    ...
    if (passData.newPassword && passData.newPasswordConfirm && passData.newPassword !== passData.newPasswordConfirm) {
      passErrors.newPasswordConfirm = `Пароли не совподают.`;
    }
  ...
  async updatePassword() {
    ...
    try {
      const token = (await ProfileService.passwordChange(passData)).data;
      ...
      observer.emit('alert', {
        title: `Ошибка`,
        message: `Не смог обновить пароль`,
      });
      this.setState({ passwordLoading: false });
    }
  }
  ...
  async submit(noCallback = false, sectionName = 'loading') {
    ...
    if (valid) {
      try {
        ...
        _data.type = parseInt(data.type.key);
        ...
        if (e.additionalInfo) {
          for (const err of e.additionalInfo) {
            errors[err.field] = t.error('requiredField');
          }
        }
