...
const LoginView = (props) => {
  ...
  const login = useCallback(async () => {
    ...
    try {
      ...
      const redirect = (localStorage.getItem('redirectUrl') ? localStorage.getItem('redirectUrl') : '')
      if (redirect) {
        localStorage.removeItem('redirectUrl')
      }
      if (APP == 'enter') {
        window.location.href = hostForRedirectAfterRegistration() + redirect.replace('/', '');
      } else {
  ...
  const validateEmail = useCallback((val) => {
    ...
    newErrors.username = !emailValid && !phoneValid ? t.error('invalidUsername') : false;
    setErrors(errors);
    setUsername(val);
  }, [username]);
        ...
        <div className={'wrap padding-top-37'}>
          ...
          {(contourCode && contourInfo) ?
            <p style={{ fontWeight: 600 }} className={''}>
              Подтвердите добавление в контур {`${CONTOUR_ROLES[contourInfo.role]} ${contourInfo.title}`},
              введя логин и пароль от вашего кабинета
            </p>
            : null}
          <div className={'form-wrapper'}>
            ...
            <VzForm.Item label={"Пароль"} className={'margin-top-16'} error={errors?.password}>
              <Input.Password
                title={t.login('password')}
                value={password}
                onChange={(e) => {
                  if (errors?.password) {
                    setErrors(prev => { return { ...prev, password: false } })
                  }
            ...
            <div className={'buttons-wrapper margin-top-31'}>
              ...
              {contourCode ?
                <Button onClick={() => history.push(`/contour-join?contourCode=${contourCode}`)} className={'wide margin-top-16'} type={'secondary'}>
                  Назад
                </Button>
                :
                <Button onClick={() => history.push('/registration')} className={'wide margin-top-16'} type={'secondary'}>
                  Регистрация
                </Button>
              }
            </div>
            <div className={'margin-top-15'}>
              <Link to={'forgot-password/email'} className={'password-restore pointer'}>
                {t.login('recoverUser')}
              </Link>
            </div>
          </div>
