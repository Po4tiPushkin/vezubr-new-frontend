...
const tableKey = `requests-active-${APP}`;
...
const RequestsActive = (props) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await RequestsService.requestActiveList({ ...QUERY_DEFAULT, ...paramsQuery });
  ...
  const oldColumns = useColumns({ dictionaries, user, reload, employees });
  ...
  const getDataFuncForExport = React.useCallback(async (config) => {
    const paramsQuery = getParamsQuery(params, dictionaries);
    await Utils.exportList(RequestsService, {
      ...QUERY_DEFAULT,
      ...paramsQuery,
