...
function RegistryDetail(props) {
  const { match, location, useReform, history, openAdd, CreateRegistry, dictionaries, user } = props;
  ...
  const HandleRemoveOrders = useCallback(async () => {
    try {
      const response = await RegistriesService.removeOrders({
        id: registryId,
        data: selectedRowKeys.map((el) => String(el)),
      });
      setShowRemove(false);
      setSelectedRowKeys([]);
  ...
  }, [registryId, registry, rowSelection]);
  ...
  const handleAddOrders = useCallback(async () => {
    try {
      const response = await RegistriesService.addOrders({ id: registryId, data: newOrders });
      setShowAdd(false);
      setNewOrders([]);
      setMastFetchData(Date.now());
    } catch (e) {
  ...
  const showRemoveHandler = () => {
    ...
    dataSource.forEach((el) => {
      if (!el.isDocumentsAcceptedByClient || !el.isCalcDocumentsAcceptedByClient) rejectedOrders.push(el.orderId);
    });
    setSelectedRowKeys(rejectedOrders);
    setShowRemove(true);
  };
  ...
  return (
    <>
      <div className="registry-detail-table-container">
        ...
        />
        <ModalWindow
          cancelButtonProps={{ disabled: !newOrders.length }}
          okText={'Добавить в Реестр'}
          okButtonProps={{ disabled: !newOrders.length }}
          children={
            <CreateRegistry
        ...
        <div className="registry__detail-actions">
          {!showRemove ? (
            <>
              <Ant.Button onClick={() => setShowAdd(true)}>Добавить рейсы</Ant.Button>
              <Ant.Button onClick={() => showRemoveHandler()}>Удалить рейсы</Ant.Button>
            </>
          ) : (
            <>
              <Ant.Button onClick={() => setShowRemove(false)}>Отмена</Ant.Button>
              <Ant.Button disabled={!selectedRowKeys.length} onClick={() => HandleRemoveOrders()}>
                Удалить рейсы
              </Ant.Button>
            </>
          )}
