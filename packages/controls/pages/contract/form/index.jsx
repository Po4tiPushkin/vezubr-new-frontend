...
// const ROUTE_CONTRACT_MAIN = createRouteWithParams(ROUTES.COUNTERPARTY, { [ROUTE_PARAMS.paramOptions]: '' });
const ROUTE_CONTRACT_AGREEMENTS = createRouteWithParams(ROUTES.CONTRACT, { [ROUTE_PARAMS.paramOptions]: 'agreements' });
const ROUTE_CONTRACT_ADDITIONAL = createRouteWithParams(ROUTES.CONTRACT, { [ROUTE_PARAMS.paramOptions]: 'additional' });
const ROUTE_CONTRACT_MARGIN = createRouteWithParams(ROUTES.CONTRACT, { [ROUTE_PARAMS.paramOptions]: 'margin' });
  ...
  return {
    ...
    items: [
      ...
      },
      {
        ...
        route: ROUTE_CONTRACT_AGREEMENTS,
        ...props,
      },
      {
        title: 'Дополнительное',
        route: ROUTE_CONTRACT_ADDITIONAL,
        ...props,
      },
      {
        title: 'Маржинальность',
        route: ROUTE_CONTRACT_MARGIN,
        show: APP === 'dispatcher',
        ...props,
      },
...
const ContractForm = (props) => {
  ...
  return (
    <>
      ...
      <Switch>
        <Route path={`/counterparty/:${match.params.id}/contract/add`}>
          <Form {...props} />
        </Route>
        ...
        <Route path={`/contract/:${match.params.id}/margin`}>
          <Margin {...props} />
        </Route>
        <Route path={`/contract/:${match.params.id}/additional`}>
          <AdditionalInfo {...props} />
        </Route>
        <Route path={`/contract/:${match.params.id}/agreements`}>
          <Agreements {...props} />
        </Route>
        <Route exact path={`/contract/:${match.params.id}/`}>
          <Form {...props} />
        </Route>
      </Switch>
    </>
