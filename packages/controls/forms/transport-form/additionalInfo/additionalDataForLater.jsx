    ...
    <>
      <VzForm.Group title={'Дополнительные параметры'}>
        <VzForm.Row>
          ...
          <VzForm.Col span={6}>
            <VzForm.Item
              label={'Наличие GPS датчика'}
              required={false}
              disabled={disabled}
              error={getFieldError(ADDITIONAL_FIELDS.hasGPSMonitoring)}
            >
              {getFieldDecorator(ADDITIONAL_FIELDS.hasGPSMonitoring, {
                initialValue: typeof values?.[ADDITIONAL_FIELDS.hasGPSMonitoring] === 'boolean' ?
                  values?.[ADDITIONAL_FIELDS.hasGPSMonitoring] :
                  undefined,
