...
function ActionEditSharingBargain(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const allProds = await getProducersForOrder(order?.orderType || order?.type, (order?.data?.points || order?.points), (order?.data?.vehicleType || order?.vehicleType))
        ...
        setSharingProducers(prodsInSharing.map(el => { return el.id }));
        setCheckedValues(prodsInSharing.map(el => { return el.id }));
  ...
  const apply = async () => {
    try {
      ...
      if (checkedValues.length) {
        await CommonService.createManualSharingForEdit(order.id, { strategyType: "bargain", appoints });
      }
      closeModal(true)
      showAlert({
  ...
  return (
    <>
      <Modal
        title={`Редактирование Шаринга для Подрядчиков на рейсе № ${order?.data?.requestNr || order?.requestNr}`}
        width={700}
        maskClosable={true}
        visible={showModal}
        centered={false}
...
ActionEditSharingBargain.propTypes = {
  order: PropTypes.object.isRequired,
  children: PropTypes.func.isRequired,
};

export default ActionEditSharingBargain;
