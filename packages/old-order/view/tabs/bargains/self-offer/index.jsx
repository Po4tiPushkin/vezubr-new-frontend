...
const SelfOffer = (props) => {
  const { offer, deleteOffer, addOffer, clientRate, bargainStatus, bargainStatuses, user = {} } = props;
  ...
  const [sumWithVat, setSumWithVat] = React.useState((offerItem?.sum + (offerItem?.sum * vatRate / 100)) || '')
  ...
  const currencyFormat = useMemo(() => new Intl.NumberFormat('ru-RU', {
    style: 'currency',
    currency: 'RUB'
  }), []);
