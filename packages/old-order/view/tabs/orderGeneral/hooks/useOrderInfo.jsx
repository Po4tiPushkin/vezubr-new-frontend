...
const useOrderInfo = ({ order, requiredDocuments, dictionaries, comment, setComment, addComment }) => {
  const { vehicleTypes } = useSelector((state) => state.dictionaries)
  const { costWithVat, vatRate } = useSelector((state) => state.user)
    ...
    const linkedOrderInfo = (linkedOrder, index) => ({
      ...
      data: [
        ...
        },
        {
          title: 'Время подачи',
          titleClass: 'thin',
          value: moment(linkedOrder?.toStartAt).format('DD MMMM, YYYY HH:mm'),
          comment: true,
        },
      ]
    })
  ...
  }, [order, requiredDocuments, dictionaries, costWithVat, vatRate, comment]);
