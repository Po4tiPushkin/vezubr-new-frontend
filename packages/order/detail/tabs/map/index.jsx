...
import * as Icons from '../../../form/icons';
import * as VzMap from '@vezubr/map'
import { OrderViewParkingPoint, OrderViewPointArrivalDeparture } from '../../components';
...
const OrderViewMap = () => {
  ...
  const { map: mapData, parkingsList } = order;
    ...
    <VzMap.Map maxZoom={16} className={'order-view__tab'} style={{ overflowY: 'hidden' }}>
      ...
      {addresses && addresses.length > 0 && (
        <VzMap.LayersControl position="topright">
            ...
            >
              {addresses.map((address, index) => (
                <VzMap.Marker key={index} value={address} pinName="pinNumberGray" pinText={index + 1}>
                  <LeafletTooltip>
                    <OrderViewPointArrivalDeparture {...address} />
                  </LeafletTooltip>
                </VzMap.Marker>
              ))}
            </VzMap.MarkerClusterGroup>
          ...
          {parkingsList && parkingsList.length > 0 && (
