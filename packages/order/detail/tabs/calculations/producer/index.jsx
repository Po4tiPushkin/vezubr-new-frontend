...
import { Ant, showAlert, showError, VzEmpty, WhiteBox, Loader, showConfirm } from '@vezubr/elements';
...
import { ADDITIONAL_TIME_ARTICLES, ALL_TIME_ARTICLES, MINIMAL_TIME_ARTICLES, SENDABLE_TIME_ARTICLES } from '../../../../form/constants';
...
const OrderViewTabCalculationProducer = (props) => {
  const { order = {}, reload: reloadInput } = useContext(OrderViewContext);
  const { isAcceptingAvailable } = props;
  const history = useHistory();

  const { orderServices, documentTypes, orderDocumentCategories } = useSelector((state) => state.dictionaries);
  const { calculationProducer, strategyType, republishStrategyType, orderUiState, requiredProfit, requiredMargin } = order;
  const {
  ...
  const [workTime, setWorkTime] = useState(
    tariffDynamicFinalCalculation?.details?.reduce((acc, item) => {
      if (
        acc &&
        !acc[item.article] &&
        (MINIMAL_TIME_ARTICLES.includes(parseInt(item.article))
          ? !Object.keys(acc).find((item) => MINIMAL_TIME_ARTICLES.includes(parseInt(item)))
          : true) &&
        (ADDITIONAL_TIME_ARTICLES.includes(parseInt(item.article))
          ? !Object.keys(acc).find((item) => ADDITIONAL_TIME_ARTICLES.includes(parseInt(item)))
          : true)
      ) {
        acc[item.article] = item.workTime;
      }
      return acc;
    }, {}),
  );
  ...
  const finalizeOrder = useCallback(async (request) => {
    setPending(true);
    try {
      setCalculationEmpty(false);
      await OrderService.finalize(request);
      showAlert({
  ...
  const releaseOrder = useCallback(
    async (dates) => {
      ...
      const newDetails = paymentDetailsRef.current.map((el) => ({
        ...el,
        article: Number(el.article),
        costPerItem: Math.ceil(el.costPerItem),
        workTime: el.article == 1319 ? workTime?.[el.article] : null,
      }));

      if (
        orderWorkTime &&
        tariffDynamicFinalCalculation?.tariffType === 1 &&
        paymentDetailsRef?.current?.find((item) => ALL_TIME_ARTICLES.includes(parseInt(item.article)))
      ) {
        requestUpdated = {
          ...requestUpdated,
          ...orderWorkTime,
      ...
      if (paymentDetailsRef.current) {
        requestUpdated.replacementServiceItems = newDetails;
        requestUpdated.replacementServiceItems = requestUpdated.replacementServiceItems.filter(
          (item) => !ALL_TIME_ARTICLES.includes(item.article) || SENDABLE_TIME_ARTICLES.includes(item.article),
        );
        if (APP == 'dispatcher') {
          let cancel = false;
          const currentTotal = requestUpdated.replacementServiceItems?.reduce((acc, item) => {
            acc += item.costPerItem * item.quantity
            return acc
          }, 0)
          const currentMargin = order?.calculationClient?.finalCalculation?.costWithVat - currentTotal;
          const currentProfit = (currentMargin/order?.calculationClient?.finalCalculation?.costWithVat) * 100;
          if ((currentProfit < 0 && currentMargin < 0) || ((currentMargin/100) < requiredMargin) || (currentProfit < requiredProfit)) {
            showConfirm({
              title: '',
              content: 'Расчетные показатели по маржинальности/марже по рейсу ниже минимально заданных. Проверьте пожалуйста корректность расчетов',
              okText: 'Отправить',
              cancelText: 'Отменить',
              onOk: () => {
                finalizeOrder(requestUpdated);
              },
              onCancel: () => {
                cancel = true;
              }
    ...
    [reload, orderId, calculationProducer, user, orderWorkTime, workTime],
  );
  ...
  const accept = useCallback(async () => {
    ...
    try {
      await OrderService.acceptOrderCalculation({
        orderId,
        calculationId: tariffDynamicFinalCalculation?.id,
      });
      showAlert({
    ...
    setPending(false);
  }, [orderId, reload, tariffDynamicFinalCalculation?.id]);

  const reject = useCallback(async () => {
    setPending(true);

    try {
      await OrderService.rejectOrderCalculation(orderId, tariffDynamicFinalCalculation?.id, rejectCommentRef.current);
      showAlert({
  ...
  const finalActions = () => {
    ...
    return (
      <>
        {isCanVisibleButtonMode && (
          <Ant.Button
            className={`${CLS}__button-mode margin-left-10`}
            htmlType="submit"
            type={'primary'}
  ...
  return (
    <WhiteBox className={`${CLS} order-view__tab`}>
      ...
      >
        ...
        >
          {(republishStrategyType ? republishStrategyType == 'tariff' : strategyType == 'tariff') &&
          (tariffDynamicFinalCalculation?.tariffId || tariffDynamicPreCalculation?.tariffId) ? (
            <>
              ...
              <span
                ...
                onClick={() =>
                  history.push(
                    `/tariffs/${tariffDynamicFinalCalculation?.tariffId || tariffDynamicPreCalculation?.tariffId}`,
                  )
                }
              >
                {` ${tariffDynamicFinalCalculation?.tariffId || tariffDynamicPreCalculation?.tariffId}`}
