...
import { Ant, showAlert, Loader, showError, WhiteBox, VzEmpty, showConfirm } from '@vezubr/elements';
...
const OrderViewTabCalculationClient = (props) => {
  ...
  const { calculationClient, strategyType, orderUiState, requiredProfit, requiredMargin } = order;
  const {
  ...
  const releaseOrder = useCallback(
    async (dates) => {
      ...
      if (paymentDetailsRef.current) {
        ...
        if (APP == 'dispatcher') {
          ...
          const currentMargin =  currentTotal - order?.calculationProducer?.finalCalculation?.costWithVat;
          const currentProfit = (currentMargin/currentTotal) * 100;
    ...
    [reload, orderId, paymentDetailsRef, orderWorkTime, workTime],
  );
  ...
  const accept = useCallback(async () => {
    ...
    setPending(false);
  }, [orderId, reload, calculationClient, tariffDynamicFinalCalculation?.id]);
  ...
  const reject = useCallback(async () => {
    ...
    try {
      await OrderService.rejectOrderCalculation(
        orderId,
        tariffDynamicFinalCalculation?.id,
