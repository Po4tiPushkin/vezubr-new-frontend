...
const DocumentsSending = (props) => {
  const { documentTypes, orderDocumentCategories } = useSelector(state => state.dictionaries);
  ...
  if (order?.orderUiState?.state !== 400) {
      ...
      <>
        {!loadingDocuments && documentsAvailable.length === 0 ? (
          <VzEmpty
            ...
            description={t.order('emptyDocuments')}
          />
        ) : (
          <>
...
export default DocumentsSending;
