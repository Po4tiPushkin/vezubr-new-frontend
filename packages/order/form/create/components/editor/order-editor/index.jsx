...
function OrderEditor(props) {
  ...
  const [numerationType, setNumerationType] = useState('');
  ...
  const fetchCargoPlaceAccounting = async () => {
    try {
      const { cargoPlaceAccounting, numerationType } = await ProfileServices.getContractorConfiguration();
      if (APP === 'client') {
        setNumerationType(numerationType);
      }
  ...
  const staticData = React.useMemo(
    () => ({
      ...
      orderType: staticData?.type ? staticData?.type : orderType ? orderType === 'city' ? 1 : 3 : 1,
    }),
  ...
  const lazyData = React.useMemo(
    () => ({
      ...
      ...lazyDataInput,
      orderType: orderType ? orderType === 'city' ? 1 : 3 : 1,
    }),
