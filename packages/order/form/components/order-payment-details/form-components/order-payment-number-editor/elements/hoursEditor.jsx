...
import { ADDITIONAL_TIME_ARTICLES, MINIMAL_TIME_ARTICLES, SENDABLE_TIME_ARTICLES } from '../../../../../constants';

function HoursEditor({ article: articleInput }) {
  const { workTime } = React.useContext(OrderPaymentContext);

  const article = SENDABLE_TIME_ARTICLES.includes(articleInput)
    ? Object.keys(workTime.get()).find((item) => item == articleInput)
    : ADDITIONAL_TIME_ARTICLES.includes(articleInput)
  ...
  const onChange = React.useCallback(
    (value, isHours) => {
      if (isHours) {
        workTime.set({ [article]: value * 60 + (minutes % 60) }, !SENDABLE_TIME_ARTICLES.includes(articleInput));
      } else {
        if (value <= 59) {
          workTime.set({ [article]: value + hours * 60 }, !SENDABLE_TIME_ARTICLES.includes(articleInput));
        }
      }
    },
