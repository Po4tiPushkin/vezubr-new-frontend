...
function OrderPaymentNumberEditor(props) {
  ...
  const articleForWorkTime = SENDABLE_TIME_ARTICLES.includes(article)
    ? Object.keys(workTime.get()).find((item) => {
        return item == article;
      })
    : ADDITIONAL_TIME_ARTICLES.includes(article)
  ...
  const isTimeQuantity = React.useMemo(
    () =>
      [...ADDITIONAL_TIME_ARTICLES, ...SENDABLE_TIME_ARTICLES].includes(article) && getCurrentProp === 'getQuantity',
    [article, getCurrentProp],
  );
